...
class Grid
    ...
    def configure_cells
        each_cell do |cell|
            row, coll = cell.row, cell.column

            cell.north = self[row - 1, coll]
            cell.south = self[row + 1, coll]
            cell.west = self[row, coll - 1]
            cell.east = self[row, coll + 1]
        end
    end
    ...
    def cell_coordinqtes_with_inset(x, y, cell_size, inset)
        x1, x4 = x, x + cell_size
        x2 = x1 + inset
        x3 = x4 - inset
    ...
    def to_png_with_inset(img, cell, mode, cell_size, wall, x, y, inset)
        x1, x2, x3, x4, y1, y2, y3, y4 = cell_coordinqtes_with_inset(x, y, cell_size, inset)
